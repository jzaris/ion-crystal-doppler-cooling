...
Generalized to 3D crystals (with FMM implementation) by John Zaris.
...
class ModeAnalysis:
    ...
    def __init__(self, N=19, XR=1, Vtrap=(0.0, -1750.0, -1970.0), Ctrap=1.0, 
        ...
        self.dx = []
        self.dy = []
        self.energy_eq_config = [] #added 3/16/25
    ...
    def expUnits(self):
        ...
        self.EvalsE3D = self.wz * self.Evals3D
    ...
    def run3D(self, pos):
        ...
        #self.generate_crystal()
        #print(np.shape(self.u))
        self.u = self.find_eq_pos3D(pos/self.l0,self.method)
        print("first minimization: " + str(self.pot_energy3D(self.u)))
        if self.precision_solving is True:
            if self.quiet is False:
                print("Perturbing crystal...")
            '''commented out 3/16/25
            if self.Nion <= 62:
                for attempt in np.linspace(.05, .5, 5): #50
                    self.u = self.perturb_position3D(self.u, attempt)
            #if 62 < self.Nion <= 126:
            if 62 < self.Nion <= 300:
                for attempt in np.linspace(1, 0.05, 1):
                    print(attempt)
                #for attempt in np.linspace(.01, .1, 5):
                    self.u = self.perturb_position3D(self.u, attempt)
            #if 127 <= self.Nion <= 200:
            #    for attempt in np.linspace(.05, .5, 10):
            #        self.u = self.perturb_position3D(self.u, attempt)
            if 301 <= self.Nion < 350:
                for attempt in np.linspace(0.2, .02, 5):
                    self.u = self.perturb_position3D(self.u, attempt)
            '''
            if self.quiet is False:
                pass
        ...
        self.Evals3D, self.Evects3D, self.V3D = self.calc_3D_modes(self.u)  #should this be divided by l0?
        self.r3D = self.calc_r()
        self.energy_eq_config = self.pot_energy3D(self.u) #added 3/16/25
    ...
    def run3D_eigen(self, pos):
        ...
        #self.axialEvals_raw, self.axialEvals, self.axialEvects = self.calc_axial_modes(self.u)
        #self.planarEvals, self.planarEvects, self.V = self.calc_planar_modes(self.u)
        self.Evals3D, self.Evects3D, self.V3D = self.calc_3D_modes(pos/self.l0)
        self.expUnits()  # make variables of outputs in experimental units
        #self.axial_hessian = -self.calc_axial_hessian(self.u)
        #self.planar_hessian= -self.V/2 
        #self.axial_Mmat    = np.diag(self.md)
    ...
    def pot_energy(self, pos_array):
        ...
        '''
        sources = np.array([x,y,np.zeros(self.Nion)])
        charges = np.full(self.Nion, 1.0)
        fmm_out = fmm.lfmm3d(eps=10**-12, sources=sources,charges=charges,dipvec=None,targets=None,pg=2,pgt=0,nd=1)
        #print(fmm_out.pot)
        Vc_tot = np.sum(fmm_out.pot)
        ...
        V = -np.sum((self.md * self.wr ** 2 + 0.5 * self.md - self.wr * self.wc) * (x ** 2 + y ** 2)) \
            + np.sum(self.md * self.Cw * (x ** 2 - y ** 2)) + 0.5 * Vc_tot

        return V
    
    def pot_energy3D(self, pos_array):
        """
        Computes the potential energy of the ion crystal,
        taking into consideration:
            Coulomb repulsion
        ...
        z = pos_array[2*self.Nion:3*self.Nion]
        '''
        # dx flattens the array into a row vector
        dx = x.reshape((x.size, 1)) - x
        dy = y.reshape((y.size, 1)) - y
        ...
        charges = np.full(self.Nion, 1.0)
        fmm_out = fmm.lfmm3d(eps=10**-10, sources=sources,charges=charges,dipvec=None,targets=None,pg=2,pgt=0,nd=1)
        #print(fmm_out.pot)
        Vc_tot = np.sum(fmm_out.pot)*self.q**2/(4*np.pi*8.8541878128*10**-12*self.l0)/self.E0
        ...
        V = 0.5 * (-m * wr ** 2 - q * self.Coeff[2] + q * B * wr) * np.sum((x ** 2 + y ** 2)) \
            ...
            + 0.5 * k_e * q ** 2 * np.sum(Vc)
        """
        V = np.sum(self.md*z**2) - np.sum((self.md * self.wr ** 2 + 0.5 * self.md - self.wr * self.wc) * (x ** 2 + y ** 2)) \
            + np.sum(self.md * self.Cw * (x ** 2 - y ** 2)) + 0.5 * Vc_tot
    ...
    def force_penning3D(self, pos_array):
        """
        Computes the net forces acting on each ion in the crystal;
        used as the jacobian by find_eq_pos to minimize the potential energy
        of a crystal configuration.
    ...
    def calc_r(self):
        r_vals = np.zeros(self.Nion*3)
        #print(self.Evects3D[:3*self.Nion,0].shape)
        #print(self.V3D.shape)
        for i in range(self.Nion*3):
            num= np.conj(self.Evects3D[:3*self.Nion,i*2]).T.dot(-self.V3D/2).dot(self.Evects3D[:3*self.Nion,i*2])
            den = self.md[0]*np.conj(self.Evects3D[3*self.Nion:,i*2]).T.dot(self.Evects3D[3*self.Nion:,i*2])
            #print(num[0,0])
            #print(den[0,0])
            if np.real(num[0,0]) > 10**6*np.imag(num[0,0]) and np.real(den[0,0]) > 10**6*np.imag(den[0,0]):
                r_vals[i] = np.real(num[0,0])/np.real(den[0,0])
            else:
                print("R calculation error")
        return r_vals
    ...
    def find_eq_pos3D(self, pos_array, method="bfgs"):
        """
        Runs optimization code to tweak the position vector defining the crystal to a minimum potential energy
        configuration.
        ...
        bfgs_tolerance = 1e-10
        cg_tolerance = 1e-10
        if method == "newton":

            out = optimize.minimize(self.pot_energy3D, pos_array, method='Newton-CG', jac=self.force_penning3D,
                                    hess=self.hessian_penning_3D,
                                    options={'xtol': newton_tolerance, 'disp': not self.quiet})
            print(out.success)
            print(out.message)
        if method == 'bfgs':
            out = optimize.minimize(self.pot_energy3D, pos_array, method='BFGS', jac=self.force_penning3D,
                                    options={'gtol': bfgs_tolerance, 'disp': False})  # not self.quiet})
            print(out.success)
            print(out.message)
        if method == 'cg':
            out = optimize.minimize(self.pot_energy3D, pos_array, method='CG', jac=self.force_penning3D,
                                    options={'gtol': cg_tolerance, 'disp': False})  # not self.quiet})
            print(out.success)
            print(out.message)
        if (method != 'bfgs') & (method != 'newton')& (method != 'cg'):
            print('method, '+method+', not recognized')
            exit()
        #print(out.success)
        return out.x
    ...
    def calc_3D_modes(self, pos_array):
        """Calculate Planar Mode Eigenvalues and Eigenvectors
    ...
    def perturb_position3D(self, pos_vect, strength=.1):
        """
        Slightly displaces each ion by a random proportion (determined by 'strength' parameter)
        and then solves for a new equilibrium position.
        ...
        """
        # print("U before:", self.pot_energy(u))
        unudge = self.find_eq_pos3D([coord * abs(np.random.normal(1, strength)) for coord in pos_vect])
        if self.pot_energy3D(unudge) < self.pot_energy3D(pos_vect):
            # print("Nudge successful")
            print("U After: ", self.pot_energy3D(unudge))
